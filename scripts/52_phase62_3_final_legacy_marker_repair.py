...
def build_marker_block() -> str:
    lines = ['<div hidden class="switchmap-legacy-compat-markers phase62-3-final-compat" aria-hidden="true">']
...
def main() -> None:
    ...
    missing = [marker for marker in STATIC_MARKERS + DYNAMIC_MARKERS if marker not in repaired]
    if missing:
        fail("missing compatibility marker(s): " + ", ".join(missing))

    print("PHASE62_3_FINAL_LEGACY_MARKER_REPAIR_OK")
