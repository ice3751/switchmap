...
if not ROOT.exists():
    ROOT = Path.cwd()
ROOT = ROOT.resolve()
PAYLOAD = ROOT / "payload_phase94_verification_baseline"
STAMP = datetime.now().strftime("%Y%m%d_%H%M%S")
LOG_DIR = ROOT / "logs"
LOG_DIR.mkdir(parents=True, exist_ok=True)
REPORT_JSON = LOG_DIR / f"phase94_verification_baseline_apply_{STAMP}.json"
REPORT_TXT = LOG_DIR / f"phase94_verification_baseline_apply_{STAMP}.txt"
BACKUP_ROOT = ROOT / "backups" / f"phase94_verification_baseline_{STAMP}"
BACKUP_FILES = BACKUP_ROOT / "files"
MANIFEST_JSON = BACKUP_ROOT / "manifest.json"

CHANGED_FILES = [
    Path("smoke_tests/manifest.json"),
    Path("smoke_tests/README.md"),
    Path("smoke_tests/run_smoke.py"),
    Path("inventory/management/commands/phase94_verification_baseline_check.py"),
]

OPTIONAL_EXISTING_GUARDS = [
    ["manage.py", "phase77_stabilization_check", "--output", str(LOG_DIR / f"phase94_phase77_stabilization_{STAMP}.txt")],
    ["manage.py", "phase80_alarm_normalization_check"],
    ["manage.py", "backup_storage_verify", "--strict"],
    ["manage.py", "backup_health_report", "--strict"],
]
...
def save_report() -> None:
    ...
    lines = [
        ...
        f"NO_VISIBLE_TEST_DATA_CREATED={report.get('visible_test_data_created')}",
...
def preflight() -> None:
    ...
    protected_roots = {"venv", ".git", "logs", "secrets", "staticfiles", "media", "backups", "_phase91_backup", "_phase91_quarantine"}
...
def verify_after_apply() -> None:
    ...
    run([sys.executable, "-m", "py_compile", "smoke_tests/run_smoke.py", "inventory/management/commands/phase94_verification_baseline_check.py"], "py_compile_changed")
    run([sys.executable, "manage.py", "check"], "django_manage_check")
    run([sys.executable, "manage.py", "phase94_verification_baseline_check", "--strict", "--output", str(LOG_DIR / f"phase94_verification_baseline_{STAMP}.json")], "phase94_verification_baseline_check", timeout=180)
    run([sys.executable, "smoke_tests/run_smoke.py", "--strict", "--output", str(LOG_DIR / f"phase94_smoke_runner_{STAMP}.json")], "phase94_smoke_runner", timeout=180)
    if optional_manage_command_exists("phase77_stabilization_check"):
        run([sys.executable, "manage.py", "phase77_stabilization_check", "--output", str(LOG_DIR / f"phase94_phase77_stabilization_{STAMP}.txt")], "phase77_stabilization_check", timeout=180)
    if optional_manage_command_exists("phase80_alarm_normalization_check"):
        run([sys.executable, "manage.py", "phase80_alarm_normalization_check"], "phase80_alarm_normalization_check", timeout=180)
    if optional_manage_command_exists("backup_storage_verify"):
        run([sys.executable, "manage.py", "backup_storage_verify", "--strict"], "backup_storage_verify_strict", timeout=180)
    if optional_manage_command_exists("backup_health_report"):
        run([sys.executable, "manage.py", "backup_health_report", "--strict"], "backup_health_report_strict", timeout=180)
    line("STEP_EXIT=verify_after_apply:0")
    add_step("verify_after_apply", "ok")


def main() -> int:
    line("PHASE94R3_VERIFICATION_BASELINE_START")
    line("MODE=install_readonly_tests_no_visible_test_data_no_restart_no_restore_no_ssh")
    line(f"ROOT={ROOT}")
    line("EXPECTED_RESULT=smoke_runner_sys_path_and_manifest_guard_fixed_without_visible_program_changes")
    line("RISK=file_only_changes_under_smoke_tests_and_management_command")
    ...
    try:
        ...
        line("NO_VISIBLE_TEST_DATA_CREATED=True")
        line("PHASE94R3_VERIFICATION_BASELINE_OK")
        return 0
    except Exception as exc:
        line(f"PHASE94_ERROR={type(exc).__name__}:{exc}")
        try:
            rollback()
        except Exception as rb_exc:
            line(f"PHASE94_ROLLBACK_ERROR={type(rb_exc).__name__}:{rb_exc}")
        report["final_ok"] = False
        report["error"] = f"{type(exc).__name__}: {exc}"
        save_report()
        line("PHASE94R3_FINAL_OK=False")
        line(f"REPORT_JSON={REPORT_JSON}")
        line(f"REPORT_TXT={REPORT_TXT}")
        line("SERVICE_RESTART=NO")
        line("DB_MUTATION=NO")
        ...
        line("BACKUP_WRITE=NO")
        line("NO_VISIBLE_TEST_DATA_CREATED=True")
        line("PHASE94R3_VERIFICATION_BASELINE_FAIL")
        return 1
