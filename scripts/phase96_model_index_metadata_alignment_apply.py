...
if not ROOT.exists():
    ROOT = Path.cwd()
ROOT = ROOT.resolve()
PAYLOAD = ROOT / "payload_phase96_model_index_metadata_alignment"
STAMP = datetime.now().strftime("%Y%m%d_%H%M%S")
LOG_DIR = ROOT / "logs"
LOG_DIR.mkdir(parents=True, exist_ok=True)
REPORT_JSON = LOG_DIR / f"phase96_model_index_metadata_alignment_apply_{STAMP}.json"
REPORT_TXT = LOG_DIR / f"phase96_model_index_metadata_alignment_apply_{STAMP}.txt"
BACKUP_ROOT = ROOT / "backups" / f"phase96_model_index_metadata_alignment_{STAMP}"
BACKUP_FILES = BACKUP_ROOT / "files"
MANIFEST_JSON = BACKUP_ROOT / "manifest.json"

CHANGED_FILES = [
    Path("inventory/models.py"),
    Path("inventory/management/commands/phase96_model_index_alignment_check.py"),
]
...
def verify_after_apply() -> None:
    ...
    run([sys.executable, "-m", "py_compile", "inventory/models.py", "inventory/management/commands/phase96_model_index_alignment_check.py"], "py_compile_changed")
    run([sys.executable, "manage.py", "check"], "django_manage_check")
    run([sys.executable, "manage.py", "phase96_model_index_alignment_check", "--strict", "--output", str(LOG_DIR / f"phase96_model_index_alignment_check_{STAMP}.json")], "phase96_model_index_alignment_check", timeout=180)
    run([sys.executable, "manage.py", "makemigrations", "--check", "--dry-run"], "makemigrations_check_dry_run", timeout=180)
    if (ROOT / "smoke_tests" / "run_smoke.py").exists():
        run([sys.executable, "smoke_tests/run_smoke.py", "--strict", "--output", str(LOG_DIR / f"phase96_phase94_smoke_runner_{STAMP}.json")], "phase94_smoke_runner", timeout=180)
    if (ROOT / "inventory" / "management" / "commands" / "phase77_stabilization_check.py").exists():
        run([sys.executable, "manage.py", "phase77_stabilization_check", "--output", str(LOG_DIR / f"phase96_phase77_stabilization_{STAMP}.txt")], "phase77_stabilization_check", timeout=180)
...
def main() -> int:
    line("PHASE96_MODEL_INDEX_METADATA_ALIGNMENT_START")
    line("MODE=file_only_model_metadata_alignment_no_migration_no_restart_no_restore_no_ssh")
    line(f"ROOT={ROOT}")
    line("EXPECTED_RESULT=models_meta_matches_phase77_indexes_and_makemigrations_clean")
    line("RISK=file_only_orm_metadata_no_db_schema_change")
    ...
    try:
        ...
        line("PHASE96_MODEL_INDEX_METADATA_ALIGNMENT_OK")
        return 0
    except Exception as exc:
        report["error"] = f"{type(exc).__name__}:{exc}"
        line(f"PHASE96_ERROR={type(exc).__name__}:{exc}")
        try:
            rollback()
        except Exception as rb_exc:
            report["rollback_error"] = f"{type(rb_exc).__name__}:{rb_exc}"
            line(f"PHASE96_ROLLBACK_ERROR={type(rb_exc).__name__}:{rb_exc}")
        report["final_ok"] = False
        save_report()
        line("PHASE96_FINAL_OK=False")
        line(f"REPORT_JSON={REPORT_JSON}")
        line(f"REPORT_TXT={REPORT_TXT}")
        line("SERVICE_RESTART=NO")
        line("DB_MUTATION=NO")
        ...
        line("SSH_EXECUTION=NO")
        line("BACKUP_WRITE=NO")
        line("PHASE96_MODEL_INDEX_METADATA_ALIGNMENT_FAIL")
        return 1
