...
DEFAULT_CSV = Path("inventory") / "data" / "office_port_labels_phase69.csv"
VALID_FIELDS = {"description", "connected_device", "cable_label", "outlet"}
LABEL_CODE_RE = re.compile(r"^[TND]\d{1,4}$", re.I)
...
def load_rows(path: Path) -> List[dict]:
    if not path.exists():
        raise CommandError(f"PHASE69_FAIL missing csv: {path}")
    rows: List[dict] = []
    with path.open("r", encoding="utf-8-sig", newline="") as f:
        reader = csv.DictReader(f)
        required = {"switch", "display_order", "label", "source_page"}
        if not required.issubset(set(reader.fieldnames or [])):
            raise CommandError(f"PHASE69_FAIL csv missing columns: {sorted(required)}")
        seen = set()
        for line_no, row in enumerate(reader, start=2):
            ...
            try:
                ...
                raise CommandError(f"PHASE69_FAIL invalid display_order at csv line {line_no}")
            if not sw or not LABEL_CODE_RE.match(label) or display_order < 1 or display_order > 48:
                raise CommandError(f"PHASE69_FAIL invalid row at csv line {line_no}")
            key = (sw, display_order)
            if key in seen:
                raise CommandError(f"PHASE69_FAIL duplicate port mapping at csv line {line_no}: {sw} {display_order}")
            seen.add(key)
            rows.append({"switch": sw, "display_order": display_order, "label": label, "source_page": (row.get("source_page") or "").strip(), "source_note": (row.get("source_note") or "").strip()})
    return rows
...
def find_switch(logical_name: str) -> Optional[Switch]:
    ...
    if len(matches) == 1:
        return matches[0]
    if len(matches) > 1:
        raise CommandError(f"PHASE69_FAIL multiple switch matches for {logical_name}: {', '.join(s.name for s in matches)}")
    matches = []
...
class Command(BaseCommand):
    help = "Correct office switch port labels using Cisco 3850 odd/even physical layout (Phase 69)."

    def add_arguments(self, parser):
        parser.add_argument("--csv", default=str(DEFAULT_CSV))
        parser.add_argument("--field", default="description", choices=sorted(VALID_FIELDS))
        parser.add_argument("--apply", action="store_true")
        parser.add_argument("--backup-db", action="store_true")
        parser.add_argument("--clear-old-label-codes", action="store_true")
    ...
    def handle(self, *args, **options):
        ...
        clear_old = bool(options["clear_old_label_codes"])
        rows = load_rows(csv_path)
        by_switch: Dict[str, List[dict]] = {}
        for row in rows:
            by_switch.setdefault(row["switch"], []).append(row)
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        report_dir = Path.cwd() / "reports" / f"phase69_correct_3850_port_labels_{stamp}"
        report_dir.mkdir(parents=True, exist_ok=True)
        report_path = report_dir / ("apply_report.csv" if apply else "dry_run_report.csv")
        if apply and options.get("backup_db"):
            backup_path = backup_database(Path.cwd() / "backups" / f"phase69_correct_3850_port_labels_{stamp}")
            if backup_path:
                self.stdout.write(f"PHASE69_DB_BACKUP={backup_path}")
            else:
                self.stdout.write("PHASE69_WARN db backup skipped")
            ...
            for logical_name, switch_rows in by_switch.items():
                ...
                if clear_old:
                    for port in switch.ports.exclude(display_order__in=mapped_ports):
                        old_value = getattr(port, field) or ""
                        if LABEL_CODE_RE.match(old_value.strip()):
                            summary["cleared"] += 1
                            action = "clear" if apply else "would_clear"
                            if apply:
                                setattr(port, field, "")
                                port.save(update_fields=[field, "updated_at"])
                            report_rows.append({"switch": logical_name, "display_order": port.display_order, "label": "", "source_page": "", "source_note": "clear stale T/N/D code only", "actual_switch": switch.name, "interface_name": port.interface_name, "old_value": old_value, "new_value": "", "action": action})
            if not apply:
        ...
        self.stdout.write(f"PHASE69_MODE={'APPLY' if apply else 'DRY_RUN'}")
        self.stdout.write(f"PHASE69_FIELD={field}")
        self.stdout.write(f"PHASE69_REPORT={report_path}")
        for key in ["switches", "mapped_rows", "updated", "unchanged", "missing_switch", "missing_port", "cleared"]:
            self.stdout.write(f"PHASE69_{key.upper()}={summary[key]}")
        if summary["missing_switch"] or summary["missing_port"]:
            raise CommandError("PHASE69_FAIL missing switch or port; inspect report")
        self.stdout.write("PHASE69_OK")
