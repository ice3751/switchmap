...
class NetworkEndpoint(models.Model):
    class ConnectionType(models.TextChoices):
        DIRECT_PORT = "direct_port", "Direct Port"
        BEHIND_AP = "behind_ap", "Behind AP"
        BEHIND_ROUTER = "behind_router", "Behind Router"
        BEHIND_TRUNK = "behind_trunk", "Behind Trunk"
        BEHIND_NETWORK_DEVICE = "behind_network_device", "Behind Network Device"
        UNKNOWN = "unknown", "Unknown"
    ...
    vlan = models.PositiveIntegerField(null=True, blank=True, db_index=True)
    ...
    last_seen_switch = models.ForeignKey(
        Switch,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="endpoint_last_seen_switches",
    )
    last_seen_port = models.ForeignKey(
        Port,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="endpoint_last_seen_ports",
    )
    via_device = models.ForeignKey(
        Switch,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="endpoint_via_devices",
    )
    via_device_name = models.CharField(max_length=200, blank=True)
    ssid = models.CharField(max_length=150, blank=True)
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["mac_address", "ip_address"], name="ep_mac_ip_idx"),
            models.Index(fields=["vlan", "last_seen"], name="ep_vlan_seen_idx"),
            models.Index(fields=["connection_type", "last_seen"], name="ep_conn_seen_idx"),
            models.Index(fields=["last_seen_switch", "last_seen"], name="ep_sw_seen_idx"),
            models.Index(fields=["last_seen_port", "last_seen"], name="ep_port_seen_idx"),
            models.Index(fields=["status", "is_active"], name="ep_status_active_idx"),
        ]

    def __str__(self):
        ip = self.ip_address or "-"
        vlan = self.vlan if self.vlan is not None else "-"
        return f"{self.mac_address} {ip} VLAN {vlan}"


class EndpointObservation(models.Model):
    ...
    ip_address = models.GenericIPAddressField(null=True, blank=True, db_index=True)
    vlan = models.PositiveIntegerField(null=True, blank=True, db_index=True)
    connection_type = models.CharField(max_length=40, db_index=True)
    confidence = models.PositiveSmallIntegerField(default=50)
    source_device_name = models.CharField(max_length=200, blank=True)
    source_interface = models.CharField(max_length=120, blank=True)
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["mac_address", "ip_address", "-observed_at"], name="epo_mac_ip_obs_idx"),
            models.Index(fields=["vlan", "-observed_at"], name="epo_vlan_obs_idx"),
            models.Index(fields=["switch", "-observed_at"], name="epo_sw_obs_idx"),
            models.Index(fields=["port", "-observed_at"], name="epo_port_obs_idx"),
            models.Index(fields=["connection_type", "-observed_at"], name="epo_conn_obs_idx"),
            models.Index(fields=["source", "-observed_at"], name="epo_source_obs_idx"),
        ]

    def __str__(self):
        ip = self.ip_address or "-"
        return f"{self.source} {self.mac_address} {ip}"
