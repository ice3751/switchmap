...
(function(){
    ...
    function payloadValue(port, name){
        ...
        if(name === 'neighbor') return [port.neighbor_device, port.neighbor_port].filter(clean).join(' / ');
    ...
    function buildState(root, attrName, button){
        ...
        var status = data(button,'status') || text(root, attrName, 'status');
        var statusUp = status.toLowerCase() === 'up' || status.toLowerCase() === 'connected';
        var neighbor = neighborFromButton(button) || text(root, attrName, 'neighbor');
        var source = data(button,'neighborSource') || text(root, attrName, 'neighbor_source');
        var mac = data(button,'macAddress') || text(root, attrName, 'mac_address');
        var ip = data(button,'ipAddress') || text(root, attrName, 'ip_address');
        var device = data(button,'device') || data(button,'connectedDevice') || text(root, attrName, 'device');
        var vlan = data(button,'accessVlan') || text(root, attrName, 'access_vlan');
        var seen = data(button,'discoveryLastPoll') || text(root, attrName, 'discovery_last_poll') || data(button,'snmpLastPoll') || text(root, attrName, 'snmp_last_poll') || data(button,'updatedAt') || text(root, attrName, 'updated_at');
        var currentIdentity = neighbor || device || mac || ip;
        if(statusUp && currentIdentity){
            return {kind:'current', identity:currentIdentity, source:source || 'current-db', mac:mac, ip:ip, vlan:vlan, seen:seen};
        }
        var last = lastFromButton(button);
        if(last && last.available && clean(last.identity || last.mac || last.ip)){
            return {kind:'last', identity:clean(last.identity) || clean(last.mac) || clean(last.ip), source:clean(last.source), mac:clean(last.mac), ip:clean(last.ip), vlan:clean(last.vlan), seen:clean(last.observed_at_text)};
        }
        return {kind:'none'};
    }
    ...
    function schedule(button){
        if(!button) return;
        activeButton = button;
        window.setTimeout(function(){ renderAll(button); }, 0);
        window.setTimeout(function(){ renderAll(button); }, 80);
        window.setTimeout(function(){ renderAll(button); }, 250);
        window.setTimeout(function(){ fetchAndRender(button); }, 350);
    }
