...
(function(){
    ...
    function effectiveLastConnectionFromDataset(d){
        // PHASE79_6_3_LAST_CONNECTED_CURRENT_OR_HISTORY_FIX
        function clean(v){
        ...
        const statusText = clean(d.status);
        const isUp = statusText.toLowerCase() === 'up';
    ...
    function setLastConnection(root, attrName, last){
        ...
        try{
            last = last || {};
            const statusText = cleanLocal(last.status_after) || readText('[' + attrName + '="status"]');
            const fallbackNeighbor = readText('[' + attrName + '="neighbor"]');
            const fallbackSource = readText('[' + attrName + '="neighbor_source"]');
            const fallbackMac = readText('[' + attrName + '="mac_address"]');
            const fallbackIp = readText('[' + attrName + '="ip_address"]');
            const identity = cleanLocal(last.identity) || cleanLocal(last.neighbor) || cleanLocal(last.connected_device) || cleanLocal(last.device) || cleanLocal(last.mac) || cleanLocal(last.ip) || fallbackNeighbor || fallbackMac || fallbackIp;
            const source = cleanLocal(last.neighbor_source) || cleanLocal(last.source) || fallbackSource;
            const mac = cleanLocal(last.mac) || fallbackMac;
            const ip = cleanLocal(last.ip) || fallbackIp;
            const vlan = cleanLocal(last.vlan) || readText('[' + attrName + '="access_vlan"]');
            ...
            const currentByStatus = statusText.toLowerCase() === 'up' && identity;
            const real = Boolean(identity || source || mac || ip || vlan || observed) && !(rawType.toLowerCase() === 'no history' && !identity && !mac && !ip);
            if(!real){ renderEmpty(box, titleEl, typeEl); return; }

            const isCurrent = Boolean(currentByType || currentByStatus);
            ...
            if(cleanLocal(last.neighbor) && cleanLocal(last.neighbor) !== identity) add('Neighbor', last.neighbor, true);
