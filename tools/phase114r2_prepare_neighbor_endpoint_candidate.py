...
PHASE = "PHASE114R2_NEIGHBOR_ENDPOINT_UI_GUARD_CANDIDATE"
TS = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
OUT = ROOT / f"SwitchMap_Phase114R2_Neighbor_Endpoint_UI_Guard_CANDIDATE_{TS}"
FILES = OUT / "files"
REPORT = OUT / "report"
PREVIEW = OUT / "preview"
ZIPDIR = OUT / "zip"
...
def patch_urls(text: str) -> tuple[str, list[str]]:
    ...
    if 'name="endpoint_search_page"' not in text and 'name="endpoint_search"' in text:
        ...
        new = old + '\n    path("endpoints/", view_required(endpoint_views.endpoint_search_view), name="endpoint_search_page"),  # Phase114R2 URL alias'
        if old in text:
            text = text.replace(old, new, 1)
            changes.append("endpoint_search_page alias added")
        else:
            changes.append("endpoint_search_page alias NOT added: exact pattern missing")
    if 'name="endpoint_search_export_csv"' not in text and 'name="endpoint_export_csv"' in text:
        ...
        new = old + '\n    path("endpoints/export.csv", view_required(endpoint_views.endpoint_export_csv_view), name="endpoint_search_export_csv"),  # Phase114R2 URL alias'
        if old in text:
            text = text.replace(old, new, 1)
            changes.append("endpoint_search_export_csv alias added")
        else:
            changes.append("endpoint_search_export_csv alias NOT added: exact pattern missing")
    return text, changes

def patch_views(text: str) -> tuple[str, list[str]]:
    changes = []
    if "PHASE114R2_NEIGHBOR_ENDPOINT_UI_GUARD" in text:
        return text, ["views already contains Phase114R2 marker"]

    block = """
# PHASE114R2_NEIGHBOR_ENDPOINT_UI_GUARD_START
try:
...
try:
    _phase114r2_original_phase79_current_connection_payload = _phase79_current_connection_payload

    def _phase79_current_connection_payload(port):
        payload = _phase114r2_original_phase79_current_connection_payload(port)
        if apply_port_connection_display_guard is None:
            return payload
        return apply_port_connection_display_guard(port, payload)
except NameError:
    pass
# PHASE114R2_NEIGHBOR_ENDPOINT_UI_GUARD_END
"""
    text = text.rstrip() + "\n\n" + block + "\n"
    changes.append("Phase114R2 wrapper appended at EOF")
    return text, changes

def make_preview_html() -> str:
    return """<!doctype html>
<html lang="fa" dir="rtl">
<head><meta charset="utf-8"><title>Phase114R2 Preview</title>
<style>
...
.bad{border-color:#fecaca;background:#fff7f7}.warn{border-color:#fde68a;background:#fffdf3}.ok{border-color:#bbf7d0;background:#f7fff9}
h2{font-size:16px;margin:0 0 10px}.k{color:#64748b;font-size:12px}.v{font:14px Consolas,monospace;direction:ltr;text-align:left;background:#f8fafc;border-radius:8px;padding:8px;margin-top:6px}.tag{display:inline-block;border-radius:999px;padding:4px 10px;font-size:12px;margin:4px;background:#e2e8f0}
</style></head>
<body>
<h1>Phase114R2 Preview</h1>
<div class="grid">
<div class="card bad"><h2>Cap-Managment / ether1</h2><div class="v">ip=172.16.25.1 | mac_count=22</div><span class="tag">Gateway ARP observed / Behind network</span></div>
<div class="card ok"><h2>CRS354 / ether47</h2><div class="v">neighbor=CAP-XL-Managment / Bridge/ether1</div><span class="tag">Physical Neighbor</span></div>
<div class="card warn"><h2>NEXUS / Ethernet1/40</h2><div class="v">alias=DOWNLINK-TO-CRS | mac_count=40</div><span class="tag">Network neighbor evidence / Aggregate behind link</span></div>
</div>
</body></html>
"""
...
def main() -> int:
    ...
    write_text(PREVIEW / "phase114r2_neighbor_endpoint_preview.html", make_preview_html())
    write_text(REPORT / "phase114r2_candidate_result.json", json.dumps(result, ensure_ascii=False, indent=2))
    write_text(REPORT / "phase114r2_candidate_report.md", f"""# {PHASE}
...
""")
    ...
    package = ZIPDIR / "SwitchMap_Phase114R2_Neighbor_Endpoint_UI_Guard_CANDIDATE.zip"
    ...
    print(f"REPORT={REPORT / 'phase114r2_candidate_report.md'}")
    print(f"RESULT={REPORT / 'phase114r2_candidate_result.json'}")
    print(f"ZIP={package}")
