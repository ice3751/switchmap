...
from inventory.endpoint_display_policy import port_visual_display_fields

register = template.Library()


def _visual_fields(port):
    """Return (and memoize) the single deterministic visual-field policy for a
    port so port_device / port_ip / port_mac / port_neighbor / port_neighbor_source
    all agree and only classify the port once per render."""
    cached = getattr(port, "_sm_visual_display_fields", None)
    ...
    try:
        ...
        visual = None
    try:
        setattr(port, "_sm_visual_display_fields", visual or {})
    except Exception:
        pass
    return visual or {}
...
def port_device(port):
    # PHASE114 FINAL: visual device comes from the deterministic display policy
    # so aggregate/behind-AP/neighbor ports never show a raw device up top.
    visual = _visual_fields(port)
    value = visual.get("device", "")
    if not value:
...
def port_title(port):
    ...
    ip = visual.get("ip_address", "") or ""
    if ip and str(ip).strip().lower() not in _EMPTY_WORDS:
        parts.append(f"IP: {ip}")

    mac = visual.get("mac_address", "") or ""
    if mac and str(mac).strip().lower() not in _EMPTY_WORDS:
...
def port_neighbor(port):
    # PHASE114 FINAL: prefer the policy neighbor (trusted evidence only).
    visual = _visual_fields(port)
    value = visual.get("neighbor", "")
    if value:
        return value
    if "neighbor" in visual:
        # Policy ran and intentionally suppressed the neighbor for this port.
        return "-"
...
@register.filter
def port_ip(port):
    # PHASE114 FINAL: only direct/neighbor visual IPs; aggregate ports get "-".
    visual = _visual_fields(port)
    if "ip_address" in visual:
        value = visual.get("ip_address", "")
        return value if str(value).strip().lower() not in _EMPTY_WORDS and value else "-"
    return _first_attr(port, _IP_FIELDS, "-") or "-"


@register.filter
def port_mac(port):
    # PHASE114 FINAL: only safely-direct visual MACs; aggregate ports get "-".
    visual = _visual_fields(port)
    if "mac_address" in visual:
        value = visual.get("mac_address", "")
        return value if str(value).strip().lower() not in _EMPTY_WORDS and value else "-"
    return _first_attr(port, _MAC_FIELDS, "-") or "-"


@register.filter
def port_neighbor_source(port):
    # PHASE114 FINAL: visual neighbor source (e.g. LLDP, "CDP / aggregate conflict").
    visual = _visual_fields(port)
    value = visual.get("neighbor_source", "")
    if value:
        return value
    return _first_attr(port, ("neighbor_source",), "") or ""


@register.filter
def port_search_terms(port):
    # PHASE114 FINAL: safe switch/port/neighbor/visual search terms for
    # client-side dashboard quick search (keeps infrastructure IPs findable).
    visual = _visual_fields(port)
    return visual.get("search_terms", "") or ""
