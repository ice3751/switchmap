(function () {
  ...
  // PHASE114 FINAL SEARCH ISOLATION:
  // This endpoint bridge must NEVER bind the dashboard quick search
  // (input[data-switch-search]) or inject endpoint rows into the dashboard
  // dropdown. It activates only on endpoint-specific UI. Dashboard quick search
  // stays owned exclusively by switchmap.js setupSearch().
  function isEndpointPage() {
    return !!(
      document.querySelector('[data-phase112r8-3-endpoint-search]') ||
      document.querySelector('form[data-endpoint-search]') ||
      document.querySelector('input[data-endpoint-search-input]') ||
      /^\/endpoints(\/|$|\?|#)/.test(window.location.pathname)
    );
  }

  function isEndpointSearchInput(input) {
    if (!input || input.tagName !== 'INPUT') return false;
    // Hard rule: never touch the dashboard switch/port quick search.
    if (input.hasAttribute('data-switch-search')) return false;
    if (input.offsetParent === null) return false;
    if (input.matches('[data-endpoint-search-input]')) return true;
    const container = input.closest('[data-phase112r8-3-endpoint-search]');
    if (container) return input.type === 'search' || input.name === 'q';
    const form = input.closest('form[data-endpoint-search]');
    if (form) return true;
    return false;
  }
  ...
  function visibleSearchInputs() {
    ...
    return Array.from(document.querySelectorAll('input')).filter(isEndpointSearchInput);
  }

  function activeSearchInput() {
    if (!isEndpointPage()) return null;
    const active = document.activeElement;
    if (isEndpointSearchInput(active)) return active;
  ...
  function start() {
    // Inert outside endpoint-specific UI so the dashboard quick search is
    // never hijacked by endpoint results.
    if (!isEndpointPage()) return;
    ensureStyle();
    bindAll();
    setInterval(bindAll, 900);
    document.addEventListener('input', function (e) {
      if (isEndpointSearchInput(e.target)) {
        bindInput(e.target);
        searchNow(e.target);
      }
    }, true);
